...
:class:`~ha_mqtt_device.BinarySensor`:
...
3. Build a :class:`~ha_mqtt_device.BinarySensor`, attach it to a
   :class:`~ha_mqtt_device.Device`, and publish its state with
   :meth:`~ha_mqtt_device.BinarySensor.set_state`. Binary sensors are
   read-only in Home Assistant: they report state but have no command topic.
...
Run it from the repository root::
    ...
    uv run python examples/binary_sensor.py
    uv run python examples/binary_sensor.py --host mqtt.example.com --port 1883 \
