...
This script walks through the lifecycle of a device that contains no
entities:
...
3. Build a :class:`~ha_mqtt_device.Device` with no entities — the device
   itself is still discoverable and announces its availability.
4. Use the device as an async context manager: entering the block publishes
   the discovery config and announces the device as available, and leaving
   the block announces it as unavailable.
5. Keep the provider running until interrupted, then remove the device with
   :meth:`~ha_mqtt_device.Device.remove`.

Run it from the repository root::

    uv run python examples/device_only.py
    uv run python examples/device_only.py --host mqtt.example.com --port 1883 \
...
async def main() -> None:
    ...
    # No entities: the device itself is still registered with Home Assistant
    # and its availability is announced on the availability topic.
