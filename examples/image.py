...
3. Build a :class:`~ha_mqtt_device.Image`, attach it to a
   :class:`~ha_mqtt_device.Device`, and publish an image with
   :meth:`~ha_mqtt_device.Image.set_image`. Images are read-only in Home
...
4. Use the device as an async context manager: entering the block publishes
   the discovery config (including the image's ``cmps`` entry) and announces
...
Run it from the repository root::
    ...
    uv run python examples/image.py
    uv run python examples/image.py --host mqtt.example.com --port 1883 \
...
#: image's ``encoding`` is ``"b64"``), so the published payload is base64 text.
PNG_B64 = (
...
async def main() -> None:
    ...
    async with provider:
        ...
        async with device:
            ...
            # Publish the image: base64-encoded payload to ~/camera/image.
