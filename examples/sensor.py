...
3. Build a :class:`~ha_mqtt_device.Sensor`, attach it to a
   :class:`~ha_mqtt_device.Device`, and publish its value with
   :meth:`~ha_mqtt_device.Sensor.set_state`. Sensors are read-only in Home
   Assistant: they report a value but have no command topic.
...
Run it from the repository root::
    ...
    uv run python examples/sensor.py
    uv run python examples/sensor.py --host mqtt.example.com --port 1883 \
...
async def main() -> None:
    ...
    async with provider:
        ...
        async with device:
            ...
            # Publish the sensor's value: numbers are converted to strings
            # ("21.5") on ~/temperature/state. Home Assistant reflects these
            # changes immediately.
